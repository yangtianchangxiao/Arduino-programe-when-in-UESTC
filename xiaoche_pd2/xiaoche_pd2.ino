...
void loop() {
  ...
  {
     err_now=AngleY-AngleY_last;
     err_sum=err_sum+err_now;
    value_back=(int)(-(kp_back*err_now+kd_back*(err_now-err_last)));
    value_back=abs(value_back);
    if(AngleY>0)
    sign=1;
    else
    sign=-1;
    value_back=(int)(sign*value_back/*/kp_back/35*15*/+86);//
